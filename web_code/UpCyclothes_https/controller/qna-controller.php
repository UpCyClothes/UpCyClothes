...
function isNewQNA(){
  ...
  $mysqli->set_charset('utf8');
  if($mysqli){
      $sql = "SELECT * FROM messenger WHERE designerID='".$user_id."' and readmark = $readmark ";
      $result = mysqli_query($mysqli,$sql);
      $array = mysqli_fetch_array($result);
      if(mysqli_num_rows($result)==0){
        return -1;
...
function getTitleListJson($designer){
  ...
  session_start();
  $user_id = $_SESSION['userId'];
  $q1 = "SELECT messengerID, messageTitle FROM messenger WHERE userID='".$user_id."' and designerID='".$designer."'";
  $result = mysqli_query($mysqli, $q1);
...
function getTitleList($designer){
  ...
  if($mysqli){
    ...
    $q1 = "SELECT messageTitle FROM messenger WHERE designerID='".$designer."'"."and userID='".$user_id."' ORDER BY messageDate DESC";
      $result = mysqli_query($mysqli,$q1);
      if(mysqli_num_rows($result)==0){
        return -1;
      }else{
...
function getIDList($designer){
  ...
  if($mysqli){
      ...
      $q1 = "SELECT messengerID FROM messenger WHERE designerID='".$designer."'"."and userID='".$user_id."' ORDER BY messageDate DESC";
      $result = mysqli_query($mysqli,$q1);
      if(mysqli_num_rows($result)==0){
        return -1;
      }else{
...
function getmessengerIDList(){
  ...
  if($mysqli){
      ...
      $q1 = "SELECT messengerID FROM messenger WHERE designerID='".$user_id."'";
...
function getqnaList(){
  ...
  if($mysqli){
      ...
      $q1 = "SELECT messageTitle FROM messenger WHERE designerID='".$user_id."'";
