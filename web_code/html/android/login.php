...
//  echo "decrypted pw : ". $decrypted_pw. "<br />\n";
  ...
  if ($ID != ""){
    ...
    $idchecking = mysqli_query($connect,"SELECT * FROM user WHERE userid='$ID'");
  	...
  	$sql = "SELECT userpw, nickname, usertype FROM user WHERE userid='$ID'";
    $result = mysqli_query($connect,$sql);
    $row =mysqli_fetch_array($result);

    //해당하는 사용자의 비밀번호를 복호화 한 것과 입력된 비밀번호가 같으면 로그인 성공!
    if(Decrypt($row["userpw"],$secret_key,$secret_iv) == $password){
      echo "{\"status\":\"OK\",\"memo\":\"1\",\"nickname\":\"$row[nickname]\",\"usertype\":\"$row[usertype]\"}";
